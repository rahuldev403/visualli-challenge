...
export default function HistoryList({ items, activeId, onSelect }: HistoryListProps) {
  return (
    <section aria-label="Previous mindmaps" className="flex min-h-0 flex-col">
      <h2 className="mb-2 border-b-2 border-line pb-1 font-pixel text-[8px] uppercase tracking-widest text-muted">
        History ({items.length})
      </h2>

      {items.length === 0 ? (
        <p className="font-terminal text-base leading-tight text-muted">
              ...
              <li key={item.id}>
                <button
                  ...
                  className={`w-full border-2 border-line px-2 py-1.5 text-left transition-colors ${
                    isActive ? "bg-accent text-line" : "bg-inset text-ink hover:bg-node-hover"
                  }`}
                >
                  <span className="block truncate font-pixel text-[7px] leading-relaxed">
                    {item.title}
                  </span>
                  <span
                    className={`block font-terminal text-sm ${
                      isActive ? "text-line" : "text-muted"
                    }`}
                  >
