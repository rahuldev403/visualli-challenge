...
class MockProvider {
        ...
        switch (request.task) {
            ...
            case "detail": {
                ...
                return JSON.stringify({
                    ...
                    connections: base.connections.filter((c) => ids.includes(c.from) && ids.includes(c.to)),
                });
            }
            case "expansion": {
                const parent = context.parent;
                if (!parent)
                    throw new errors_1.ProviderError("Mock expansion requested without a parent node");
                const fixture = (0, fixtures_1.matchFixture)(sourceText);
                const canned = fixture?.expansions[parent.id];
                return JSON.stringify(canned ?? (0, fixtures_1.syntheticExpansion)(parent, context.idPrefix ?? "x"));
            }
        }
    }
    /** A matched fixture if the text looks like a demo input, else a synthetic map. */
    baseMindmap(sourceText) {
        return (0, fixtures_1.matchFixture)(sourceText)?.mindmap ?? (0, fixtures_1.syntheticMindmap)(sourceText);
    }
    /**
     ...
     */
    deliberatelyBrokenPayload(request) {
        if (request.task === "detail") {
            ...
            return {
                ...
                connections: [
                    { from: ids[0], to: "hallucinated", label: "invented" },
                    { from: ids[0], to: ids[0], label: "self loop" },
                ],
            };
        }
